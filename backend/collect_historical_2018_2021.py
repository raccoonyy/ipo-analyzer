"""
Collect historical IPO data from 38.co.kr (2018-2021)
...
def parse_ipo_html(html):
    ...
    # 10. 주간사 (Lead underwriter)
    patterns = [
        r"주간사[^<]*?</td>[^<]*?<td[^>]*>&nbsp;\s*<b>([가-힣A-Za-z0-9,·\s]+)</b>",
        r"주간사[^<]*?</td>[^<]*?<td[^>]*>&nbsp;\s*([가-힣A-Za-z0-9,·\s]+?)</td>",
    ]
    for pattern in patterns:
        match = re.search(pattern, html, re.DOTALL)
        if match:
            underwriter = match.group(1).strip()
            if underwriter and underwriter != "&nbsp;":
                data["lead_underwriter"] = underwriter
                break

    # 11. 수요예측 참여건수 (Demand forecast participants)
    patterns = [
        r"참여건수 \(단위:건\)[^<]*?</td>.*?<tr>.*?<td[^>]*?>([0-9,]+)</td>",
    ]
    for pattern in patterns:
        match = re.search(pattern, html, re.DOTALL)
        if match:
            try:
                ...
                data["demand_forecast_participants"] = int(count)
                break
            except:
                pass

    # 12. 매출액 (Revenue in million won)
    patterns = [
        r"매출액[^<]*?<td[^>]*>&nbsp;\s*([0-9,]+)\s*\(백만원\)",
        r"매출액</td>\s*<td[^>]*>&nbsp;\s*([0-9,]+)\s*\(백만원\)",
    ]
    for pattern in patterns:
        match = re.search(pattern, html)
        if match:
            try:
                ...
                data["revenue_million_krw"] = int(revenue)
                break
            except:
                pass

    # 13. 순이익 (Net income in million won)
    patterns = [
        r"순이익[^<]*?<td[^>]*>&nbsp;\s*([0-9,\-]+)\s*\(백만원\)",
        r"순이익</td>\s*<td[^>]*>&nbsp;\s*([0-9,\-]+)\s*\(백만원\)",
    ]
    for pattern in patterns:
        match = re.search(pattern, html)
        if match:
            try:
                ...
                data["net_income_million_krw"] = int(income)
                break
            except:
                pass

    # 14. 영업이익률 (Operating margin %)
    patterns = [
        r"영업이익률</td>\s*<td[^>]*?>([0-9.\-]+)\s*%",
    ]
    for pattern in patterns:
        match = re.search(pattern, html, re.DOTALL)
        if match:
            try:
                data["operating_margin_pct"] = float(match.group(1))
                break
            except:
                pass

    # 15. 당기순이익률 (Net margin %)
    patterns = [
        r"당기순이익률</td>\s*<td[^>]*?>([0-9.\-]+)\s*%",
    ]
    for pattern in patterns:
        match = re.search(pattern, html, re.DOTALL)
        if match:
            try:
                data["net_margin_pct"] = float(match.group(1))
                break
            except:
                pass
    ...
    # 16. 부채비율 (Debt ratio %)
    patterns = [
        r"부채비율</td>\s*<td[^>]*?>([0-9.\-]+)\s*%",
    ]
    for pattern in patterns:
        match = re.search(pattern, html, re.DOTALL)
        if match:
            try:
                data["debt_ratio_pct"] = float(match.group(1))
                break
            except:
                pass

    # 17. 유동비율 (Current ratio %)
    patterns = [
        r"유동비율</td>\s*<td[^>]*?>([0-9.\-]+)\s*%",
    ]
    for pattern in patterns:
        match = re.search(pattern, html, re.DOTALL)
        if match:
            try:
                data["current_ratio_pct"] = float(match.group(1))
                break
            except:
                pass

    # 18. 매출액증가율 (Revenue growth %)
    patterns = [
        r"매출액증가율</td>\s*<td[^>]*?>([0-9.\-]+)\s*%",
    ]
    for pattern in patterns:
        match = re.search(pattern, html, re.DOTALL)
        if match:
            try:
                data["revenue_growth_pct"] = float(match.group(1))
                break
            except:
                pass

    # 19. 영업이익증가율 (Operating income growth %)
    patterns = [
        r"영업이익증가율</td>\s*<td[^>]*?>([0-9.\-]+)\s*%",
    ]
    for pattern in patterns:
        match = re.search(pattern, html, re.DOTALL)
        if match:
            try:
                data["operating_income_growth_pct"] = float(match.group(1))
                break
            except:
                pass
...
def is_target_year(listing_date, target_years=[2018, 2019, 2020, 2021]):
...
def main():
    print("=" * 80)
    print("38.CO.KR HISTORICAL IPO DATA COLLECTION (2018-2021)")
    print("=" * 80)
    print("Target years: 2018, 2019, 2020, 2021")
    print("Rate limit: 0.6s per request")
    print("HTML Cache: data/cache/38_html/")
    print()
    sys.stdout.flush()

    # IPO number range to search (estimated for 2018-2021)
    start_no = 700
    end_no = 1399
    ...
    if collected_data:
        ...
        columns = [
            ...
            "lockup_ratio",
            "shares_offered",
            "demand_forecast_participants",
            "revenue_million_krw",
            "net_income_million_krw",
        ...
        # Save to CSV
        output_file = "data/raw/38_historical_2018_2021.csv"
